...
/*
Nesta instrução JPQL, usamos a função de agregação avg (que calcula a média)
 */
    @Query("select avg(m.quantidadeReproducoes) from Musica m")
    fun mediaReproducoes(): Double

/*
Para usarmos projections com interfaces,
é OBRIGATÓRIO o uso de SQL Nativa e não JPQL
(para isso, fizemos nativeQuery = true)
 */
    @Query(value = "SELECT id, nome, quantidade_reproducoes FROM musica",
    nativeQuery = true)
    fun findAllSimples(): List<MusicaSimplesResposta>
}

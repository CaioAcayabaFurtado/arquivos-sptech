...
import sptech.projeto05.dto.MusicaSimplesResposta
...
Para que faça o que tem que fazer com JPA
    ...
    fun getPorNome(@RequestParam nome:String): ResponseEntity<List<Musica>> {
 ...
 */
    ...
    fun relatorio() : ResponseEntity<RelatorioResposta> {
        ...
        val mediaReproducoes = repositorio.mediaReproducoes()

        val resposta = RelatorioResposta(total, proprias, improprias, mediaReproducoes)
    ...
    @GetMapping("/simples")
    fun getSimples(): ResponseEntity<List<MusicaSimplesResposta>> {

        return ResponseEntity.status(200)
                .body(repositorio.findAllSimples())
    }

    @PatchMapping("/foto/{id}")
    fun patchFoto(@PathVariable id: Int, @RequestBody foto: ByteArray): ResponseEntity<Void> {
        val musicaEncontrada = repositorio.findByIdOrNull(id)
        musicaEncontrada!!.foto = foto
        repositorio.save(musicaEncontrada)
        return ResponseEntity.status(200).build()
    }

    // O produces determina o content-type da resposta
    // Devemos usar agum content-type padronizado
    @GetMapping( value = ["/foto/{id}"], produces = ["image/png", "image/jpeg"])
    fun getFoto(@PathVariable id: Int): ResponseEntity<ByteArray> {
        val musicaEncontrada = repositorio.findByIdOrNull(id)
        return if (musicaEncontrada?.foto != null) {
            ResponseEntity.status(200).body(musicaEncontrada.foto)
        } else {
